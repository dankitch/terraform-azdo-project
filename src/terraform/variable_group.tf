resource "azuredevops_variable_group" "azure_credentials" {
  ...
  description  = "Azure credentials for the ${var.environments[count.index]} environment."
  allow_access = true
...
resource "azuredevops_variable_group" "azure_backends" {
  count       = length(var.environments)
  project_id  = azuredevops_project.main.id
  name        = "azure-backend-${var.environments[count.index]}"
  description = "Azure Terraform backend for the ${var.environments[count.index]} environment."
